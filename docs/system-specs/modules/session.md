...
- **Reaping abandoned agent scopes** (`session_scope_reap.py`,
  ...
  only when ALL hold: (i) no member PID is tracked or a live provider; (ii)
  EVERY member is this install's own — it carries the `KIROCREW_SPAWNED`
  marker, or its `ppid` chain reaches a marker-bearing member without leaving
  the scope's member set (ownership is by tree: env-clearing grandchildren such
  as `chrome-headless` renderers under a playwright `node` daemon carry no
  marker, and that leaked daemon tree is the multi-GB survivor users report);
  an unreadable `environ` fails closed to "not reclaimable"; (iii) the group
  leader is dead OR the
  ...
  re-reads `cgroup.procs`, opens a pidfd before re-deriving tree ownership, and
  ...
  (`sandbox._probe_cgroup_scope`). A scope whose marker-bearing members have all
  died cannot prove ownership of surviving env-cleared descendants and is never
  reclaimed; old skipped scopes are summarized at INFO by stable reason category,
  making that accepted fail-closed residual operator-visible. Stale
