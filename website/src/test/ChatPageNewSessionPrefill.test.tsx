...
import chatReducer, { switchSlot, endLocalTurn } from '../store/chatSlice'
...
describe('App SDK chat launch intent', () => {
  ...
  it.each([
    ...
    launch({ message: PROMPT, slotKey: 'chat-other', autoSend })
    await act(async () => { navigateInTest('/chat?sid=chat-other') })
    await waitFor(() => expect(screen.getByTestId('sid-error').textContent).toContain(`Couldn't open "${title || 'chat-other'}". Try again.`))
