...
for (const theme of ['dark', 'light']) {
  ...
  await page.waitForTimeout(400)
  if (!/Cancel and remove the instance/i.test(await cancel.innerText())) {
    fail(`${theme}: the visible Cancel copy on an unregistered launch must say it removes the instance`)
  }
  ...
  // The card is the badge row's grandparent (<Card><div class="flex">…).
  const card = cancel.locator('xpath=../..')
  await page.screenshot({
