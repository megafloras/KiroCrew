...
def _discard_cancelled_plan_queue_entries(slot: "_ChatSlot") -> None:
    """Drop queue work owned by a revoked plan, preserving plain user input."""
    retry_id = slot._stage_delivery_retry_queue_id
    slot._queue[:] = [
        entry
        for entry in slot._queue
        if entry.get("id") != retry_id
        and not _is_plan_approval_entry(entry)
        and entry.get("kind") not in STAGE_DELIVERY_KINDS
    ]
...
async def _exit_cancelled_plan(state: "DashboardState", slot: "_ChatSlot") -> None:
    ...
    if slot._queue:
        _discard_cancelled_plan_queue_entries(slot)
...
def _queue_consumed_stage_resume(
    ...
    *,
    ...
    if (
        slot._stage_delivery_pending is None
        or not slot._stage_delivery_consumed
        or not (slot._last_turn_auth_required or slot._stage_continuation_required)
    ):
        return False
    ...
    if _exact_stage_delivery_retry(slot) is None:
        slot._stage_delivery_retry_queue_id = slot.queue_insert(
            0,
            _MANUAL_RESUME_MSG,
    ...
    while not _orchestration_stopped(
        ...
        try:
            ...
            for parent_key in parent_keys:
                reports_observed = (
                    bool(await manager.wait_for_parent_reports(parent_key))  # type: ignore[attr-defined]
                    or reports_observed
                )
            if reports_observed:
                clean_passes = 0
...
async def _stage_loop(
    ...
    def _preserve_interrupted_stage() -> None:
        ...
        if _active_stage_num is None or slot._stage_delivery_pending is not None:
            return
        slot._stage_delivery_pending = _active_stage_num
        slot._stage_delivery_consumed = _active_stage_consumed
        slot._stage_continuation_required = _active_stage_consumed
    ...
    try:
        ...
        for stage_idx in range(start_idx, total):
            ...
            try:
                ...
                def _record_stage_turn_consumed(consumed: bool) -> None:
                    ...
                    if slot._stage_delivery_pending == stage_num:
                        slot._stage_delivery_consumed = consumed
                        if consumed:
                            slot._stage_delivery_retry_queue_id = ""
            ...
            slot._stage_delivery_consumed = _stage_turn_consumed
            ...
            # Execution completion and parent delivery are separate events. The
            # reusable settlement function is the single owner of report waits,
            # live turn joins, queued completion drains, and failure halts.
            if not await _settle_stage_delivery(
                state,
                slot,
                tracker,
        ...
        if (
            ...
            and not _turn_live
            and not slot._last_turn_auth_required
            and slot._stage_delivery_pending is None
            and state._slots.get(slot.key) is slot
            and slot._queue
            and not slot._stopping
        ):
