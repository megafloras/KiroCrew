...
2. EVERY member is this install's own: it carries the ``KIROCREW_SPAWNED``
   marker, or its ``ppid`` chain reaches a marker-bearing member without
   leaving the scope (env-clearing grandchildren such as ``chrome-headless``
   under a playwright daemon); an unreadable ``environ`` fails closed;
...
class ReapSummary:
    ...
    reclaimed: int = 0
    skipped: int = 0
    reclaimed_units: list[str] = field(default_factory=list)
...
def _pid_has_marker(pid: int, proc_root: Path) -> bool | None:
    """Whether *pid*'s exec-time environ carries ``KIROCREW_SPAWNED``.

    Returns ``None`` when the environ cannot be read, so callers can fail closed
    ("not reclaimable") on an unreadable member -- distinct from a readable
    environ that simply lacks the marker (``False``).
    """
...
def _scope_owned_pids(pids: list[int], proc_root: Path) -> tuple[set[int], str]:
    ...
    reason = ""
    for pid in pids:
        marker = _pid_has_marker(pid, proc_root)
        if marker is None:
...
def _scope_reclaimable(
    ...
    *,
    ...
    # (ii) every member is ours -- by marker or by descent from a marked member
    # inside this scope; an unreadable environ fails closed.
...
def _pidfd_signal_owned(
    ...
    """Pin *pid*, re-verify ownership, then signal that exact process object.

    The pidfd is opened before ownership is read. A process recycled before the
    open fails ownership; one recycled after the open cannot retarget the fd.
...
def _reclaim_scope(
    ...
    *,
    ...
    signal_owned: Callable[[int, int, list[int], Path], tuple[bool, str]],
    ...
    for sig in (signal.SIGTERM, signal.SIGKILL):
        ...
        for pid in remaining:
            if pid <= 1 or pid == my_pid:
                continue
            _sent, reason = signal_owned(pid, sig, remaining, proc_root)
            if reason:
...
def reap_scopes(
    ...
    *,
    ...
    signal_owned: Callable[[int, int, list[int], Path], tuple[bool, str]] = _pidfd_signal_owned,
    ...
    for scope_dir in children:
        ...
        if cleared:
            summary.reclaimed += 1
            summary.reclaimed_units.append(unit_name)
        else:
            summary.skipped += 1
