...
def snapshot_main(
    ...
    omitted = [s for s in omissions if s["reason"] == pinned_fs.SKIP_UNREADABLE_ENTRY]
    if omitted:
        print(
            f"⚠️  Not pruning: this bundle omits {len(omitted)} entr"
            f"{'y' if len(omitted) == 1 else 'ies'} this process could not read "
            f"(see MANIFEST.json). Older bundles are kept so a complete one is not "
            f"replaced by an incomplete one."
        )
    else:
