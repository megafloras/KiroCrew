...
class _Recorder:
    ...
    def signal_owned(
        self, pid: int, sig: int, _members: list[int], _proc_root: Path
    ) -> tuple[bool, str]:
        self.kill(pid, sig)
        return True, ""
...
def test_reclaims_dead_leader_untracked(tmp_path):
    ...
    assert summary.reclaimed_units == ["run-u1.scope"]
...
def test_reclaims_env_clearing_descendants_by_tree(tmp_path):
    # Playwright shape: a marked node daemon (dead leader 300) with
    # chrome-headless children that cleared their environ. Ownership is by
    # descent, so the scope is reclaimable AND the unmarked children are
    # signalled in the fallback (systemctl stop left them behind).
    slice_dir = tmp_path / "slice"
    proc = tmp_path / "proc"
    _make_proc(proc, 301, pgrp=300, comm="node_real")  # marked
    ...
    assert summary.reclaimed_units == [] and summary.skipped == 1  # fake scope never empties
    assert rec.stopped == ["run-u1.scope"]
...
def test_pidfd_pin_precedes_ownership_and_signal(monkeypatch, tmp_path):
    ...
    sent, reason = r._pidfd_signal_owned(201, signal.SIGTERM, [201], tmp_path)
...
def test_pidfd_process_lookup_is_quiet(monkeypatch, tmp_path):
    ...
    assert r._pidfd_signal_owned(201, signal.SIGTERM, [201], tmp_path) == (False, "")
